  ...
  const action = async (body: Record<string, unknown>, success = "") => {
    ...
    try {
      ...
      if (body.settings && ['enqueue', 'resume', 'retry'].includes(String(body.action))) {
        ...
        const status = await readApiJson<{ seriesVideoModelSelection?: boolean; seriesSingleShotImages?: boolean; h3DasiwaHybridPruned4?: boolean; h3HybridMultiReference?: boolean; storyFrameAlignedDirection?: boolean; seriesVisualStyleSelection?: boolean; seriesVideoModelRerun?: boolean }>(
        ...
        if (productionSettings.videoProvider === 'comfyui' && (!status.h3DasiwaHybridPruned4 || !status.h3HybridMultiReference || !status.storyFrameAlignedDirection)) throw new Error('请更新 Companion 后开始 H3 制作，新版会按首帧锚定 Hybrid 编排多图、动作与机位。');
  ...
  const redoVisuals = async () => {
    ...
    try {
      ...
      const status = await readApiJson<{
        ...
        h3DasiwaHybridPruned4?: boolean; h3HybridMultiReference?: boolean; storyFrameAlignedDirection?: boolean;
      ...
      if (!status.seriesVisualStyleSelection || (productionSettings.videoProvider === 'comfyui' && (!status.h3DasiwaHybridPruned4 || !status.h3HybridMultiReference || !status.storyFrameAlignedDirection))) throw new Error('请更新 Companion 后重做，以便使用新风格与首帧锚定 H3 Hybrid 四步。');
