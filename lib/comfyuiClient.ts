...
export const SEGMENT_VIDEO_COMPANION_MIN_VERSION = [0, 1, 227] as const;
...
export async function assertH3ProductionSupport(settings?: Partial<ComfyUISettings>): Promise<void> {
  ...
  if (!status.h3HybridMultiReference || !companionVersionAtLeast(String(status.version || ''), SEGMENT_VIDEO_COMPANION_MIN_VERSION)) {
    throw new Error(`H3 多图已统一使用首帧锚定 Hybrid，请先更新 Companion 至 v${SEGMENT_VIDEO_COMPANION_MIN_VERSION.join('.')} 或更高版本`);
  }
}
